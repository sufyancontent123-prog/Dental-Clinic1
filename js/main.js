...
} else if ($(".newsletter-form").length) {
    $(".newsletter-form").on("submit", function(event){
        event.preventDefault();
    });
}
function callbackFunction(resp){if(resp.result==="success"){formSuccessSub();}
else{formErrorSub();}}
function formSuccessSub(){if($(".newsletter-form").length){$(".newsletter-form")[0].reset();}submitMSGSub(true,"Thank you for subscribing!");setTimeout(function(){$("#validator-newsletter").addClass('hide');},4000)}
...
if($("#days").length){setInterval(function(){makeTimer();},1000);}
if($.fn.niceSelect && $('select').length){$('select').niceSelect();}
if($.fn.magnificPopup && $('.popup-youtube').length){$('.popup-youtube').magnificPopup({disableOn:320,type:'iframe',mainClass:'mfp-fade',removalDelay:160,preloader:false,fixedContentPos:false});}
if($.fn.appear && $('.odometer').length){$('.odometer').appear(function(e){var odo=$(".odometer");odo.each(function(){var countNumber=$(this).attr("data-count");$(this).html(countNumber);});});}
$(window).on('scroll',function(){var scrolled=$(window).scrollTop();if(scrolled>600)$('.go-top').addClass('active');if(scrolled<600)$('.go-top').removeClass('active');});$('.go-top').on('click',function(){$("html, body").animate({scrollTop:"0"},500);});$(window).on('load',function(){if(typeof WOW!=='undefined'&&$(".wow").length){var wow=new WOW({boxClass:'wow',animateClass:'animated',offset:20,mobile:true,live:true,});wow.init();}});$(window).on('load',function(){$('.preloader, .innova-preloader').addClass('preloader-hidden').fadeOut(300);});}(jQuery));